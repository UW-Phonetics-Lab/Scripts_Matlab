...
function hearingTest3()
    %GLOBAL VARIABLES
    %the frequencies you want to test, their amplitudes, and a random assignment to either left or right
    subjectCode = '';
    codedFilename = '';
    frequencyVector = [250 500 1000 2000 4000 8000];
    amplitudeVector = [0.05 0.01 0.005];
    directionVector = zeros(1,length(frequencyVector));
    a = rand(size(directionVector));
    for b = 1:length(a)
        directionVector(b) = round(a(b)); %a random vector of zeros (LEFT) and ones (RIGHT)
        if (directionVector(b) == 0)
            directionVector(b) = -1; %recoding left as -1 instead of 0 so that the inverse direction vector will work
        end
    end
    inverseDirectionVector = -1 .* directionVector;
%    row1 = repmat(frequencyVector,1,6); %repeat the freq & amp vectors 6 times (they play out of each ear 3 times)
%    row2 = repmat(amplitudeVector,1,6);
    
    row1 = [250 500 1000 2000 4000 8000 250 500 1000 2000 4000 8000 250 500 1000 2000 4000 8000 250 500 1000 2000 4000 8000 250 500 1000 2000 4000 8000 250 500 1000 2000 4000 8000];
    row2 = [0.05 0.05 0.05 0.05 0.05 0.05 0.01 0.01 0.01 0.01 0.01 0.01 0.005 0.005 0.005 0.005 0.005 0.005 0.05 0.05 0.05 0.05 0.05 0.05 0.01 0.01 0.01 0.01 0.01 0.01 0.005 0.005 0.005 0.005 0.005 0.005];
    
    
    row3 = horzcat(directionVector,inverseDirectionVector,directionVector,inverseDirectionVector,directionVector,inverseDirectionVector);
